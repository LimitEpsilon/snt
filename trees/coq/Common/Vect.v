...
Definition index_of_nat_lt (sz n: nat) (LT : n < sz) : index sz :=
  match index_of_nat sz n as idx return idx <> None -> _ with
  | Some i => fun _ => i
  | None => fun NEQ => False_rect _ (NEQ eq_refl)
  end (fun EQ => let contra := index_of_nat_none_ge _ _ EQ in ltac:(abstract lia)).
...
Fixpoint vect (T : Type) n : Type :=
  match n with
  | 0 => unit
  | S n => (T * @vect T n)
  end.

Definition vect_hd {T n} (v: vect T (S n)) : T := fst v.
...
Definition vect_tl {T n} (v: vect T (S n)) : vect T n := snd v.

Definition vect_nil {T} : vect T 0 := tt.

Definition vect_cons {T n} (t: T) (v: vect T n) : vect T (S n) := (t, v).

Definition vect_rect {A : Type}
  (P : forall n, vect A n -> Type)
  (Pnil : P _ vect_nil)
  (Pcons : forall n (a : A) (vec : vect A n), P _ vec -> P _ (vect_cons a vec))
: forall {n} (vec : vect A n), P _ vec :=
  fix vrect n :=
    match n as m return forall v : vect A m, P _ v with
    | 0 => fun v => let '(tt) := v in Pnil
    | S n' => fun v => let '(hd, tl) := v in Pcons _ hd tl (vrect n' _)
    end.
...
Proof.
  unfold vect_hd, vect_tl. destruct v. reflexivity.
Qed.
...
Fixpoint vect_skipn_cast n:
  n = n - 0.
Proof. destruct n; cbn; reflexivity.  Defined.
...
Fixpoint vect_extend_end_cast sz sz':
  sz + (sz' - sz) = Nat.max sz sz'.
Proof. destruct sz, sz'; cbn; auto using vect_app_nil_cast. Defined.
...
Section Conversions.
  ...
  Fixpoint list_to_vect {A} n (l : list A) : option (vect A n) :=
    match n with
    | O => match l with nil => Some vect_nil | _ => None end
    | S n' =>
      match l with
      | nil => None
      | hd :: tl =>
        match list_to_vect n' tl with
        | Some tl' => Some (vect_cons hd tl')
        | None => None
        end
      end
    end.

  Lemma list_to_vect_to_list {A n} :
    forall v : vect A n, list_to_vect n (vect_to_list v) = Some v.
  Proof.
    induction n; simpl; auto.
    - intros []; auto.
    - intros (hd & tl). simpl. unfold vect_to_list in IHn.
      rewrite IHn. auto.
  Qed.
...
Hint Rewrite @List.length_firstn : vect_to_list_cleanup.
...
Proof.
  ...
  - intros Hinj. rewrite NoDup_nth_error; intros n1 n2 Hlt Heq.
    ...
    assert (n2 < sz) as Hlt2 by (rewrite <- (vect_to_list_length v); apply nth_error_Some; intros H; erewrite <- Heq in H; congruence).
...
Instance EqDec_vect_nil T `{EqDec T} : EqDec (unit) := _.
Instance EqDec_vect_cons A B `{EqDec A} `{EqDec B} : EqDec (A * B) := _.
...
Section Npow2.
  ...
  Lemma Npow2_gt_0 :
    forall n, (0 < N.pow 2 n)%N.
  Proof.
    intros. pose proof Npow2_ge_1. lia.
  Qed.
...
Module Bits.
  ...
  Proof.
    induction n; cbn; intros; destruct bss.
    - reflexivity.
  ...
  Section Casts.
    ...
    Definition of_2cZ sz (z: Z) : bits sz :=
      match z with
      | Z0 => zeroes sz
      | Zpos x => of_positive sz x
      | Zneg x => neg (of_N sz (N.pred (Npos x)))
      end.
    ...
    Proof.
      ...
      - reflexivity.
      - destruct b, b0, (to_N v) eqn:?, (to_N v0) eqn:?; cbn in *;
          discriminate || (rewrite (IHsz v v0) by congruence; reflexivity).
    Qed.
    ...
    Proof.
      ...
      - setoid_rewrite Nat2N.inj_succ; rewrite N.pow_succ_r'.
        destruct b, (to_N v) eqn:Heq; cbn -[N.pow N.mul N.add];
          specialize (IHsz v); rewrite Heq in IHsz; nia.
    Qed.
    ...
    Lemma to_nat_inj :
      forall sz (bs1 bs2: bits sz),
        ...
        bs1 = bs2.
    Proof. cbv [to_nat]. intros. eapply to_N_inj. Zify.zify. eauto. Qed.
