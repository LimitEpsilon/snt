...
Module Import IdentToStringImpl.
  ...
  Ltac2 varconstr_to_coq(c: constr) :=
    ...
    | Constr.Unsafe.Constant c _ => 
      let s := Std.ConstRef c in 
      let fullyqualified_path := Env.path s in 
      let end_name := Ltac2.List.last fullyqualified_path in
      ident_to_coq end_name
...
Notation "ident_to_string! a" :=
  (match mkLtac2IdentToPass with
   | a => ltac:(serialize_ident_in_context)
   end) (at level 10, a ident, only parsing).
