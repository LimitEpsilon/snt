...
Section facts.
  ...
  Proof.
  destruct a; try destruct eff; cbn; intuition;
  unfold rd_st in *; rewrite EQ in *; eauto.
  Qed.
  ...
  - intros. eapply HINT; eauto.
    eapply (interp_atom_pure_implies Pure); try eassumption.
    simpl. intros. destruct (Bits.single x');
    [eapply IHl1 | eapply IHl2]; eauto;
  ...
  Lemma nonempty_linterp_atom_pure
  : ∀ E ty (a : atom _ E ty),
    match E as h return atom Bits.bits h ty → Prop with
    | Pure => fun x =>
      ∀ (U : Update Σ) log P
        (INTERP : linterp_atom_pure St U x log P),
        ∃ ret log', P ret log'
    | Effectful => fun _ => True
    end a.
  Proof.
  ...
  Lemma nonempty_interp_atom_pure
  : ∀ E ty (a : atom _ E ty),
    match E as h return atom Bits.bits h ty → Prop with
    | Pure => fun x =>
      ∀ (U : Update Σ) P
  ...
  Lemma nonempty_linterp_atom
  : ∀ E ty (a : atom _ E ty) (U : Update Σ) log P
      (INTERP : linterp_atom St a U log P),
      ∃ ret U' log', P ret U' log'.
  Proof.
  destruct a; try destruct eff; simpl; intuition; eauto;
  repeat match goal with
  | H : ∃ _, _ |- _ => destruct H
  ...
  Lemma nonempty_interp_atom
  : ∀ E ty (a : atom _ E ty) (U : Update Σ) P
  ...
  Lemma nonempty_linterp_linear_pure
  : ∀ E ty (l : linear _ E ty),
    match E as h return linear Bits.bits h ty → Prop with
    | Pure => fun x =>
      ∀ (U : Update Σ) log P
        (INTERP : linterp_linear_pure St U x log P),
        ∃ ret log', P ret log'
    | Effectful => fun _ => True
    end l.
  Proof.
  induction l; try destruct eff; simpl; intuition;
  apply (nonempty_linterp_atom_pure Pure) in INTERP;
  repeat match goal with
  | H : ∃ _, _ |- _ => destruct H
  | H : _ |- _ => apply IHl1 in H
  | H : _ |- _ => apply IHl2 in H
  | H : _ |- _ => apply IHcont in H
  | H : context [Bits.single ?x] |- _ => destruct (Bits.single x)
  end; eauto.
  Qed.

  Lemma nonempty_interp_linear_pure
  : ∀ E ty (l : linear _ E ty),
    match E as h return linear Bits.bits h ty → Prop with
    | Pure => fun x =>
      ∀ (U : Update Σ) P
  ...
  Proof.
  induction l; try destruct eff; simpl; intuition;
  apply (nonempty_interp_atom_pure Pure) in INTERP;
  repeat match goal with
  | H : ∃ _, _ |- _ => destruct H
  | H : _ |- _ => apply IHl1 in H
  | H : _ |- _ => apply IHl2 in H
  ...
  Lemma nonempty_linterp_linear
  : ∀ E ty (l : linear _ E ty) (U : Update Σ) log P
  ...
  repeat match goal with
  | H : ∃ _, _ |- _ => destruct H
  | H : _ |- _ => apply (nonempty_linterp_atom_pure Pure) in H
  | H : _ |- _ => apply nonempty_linterp_atom in H
  | H : _ |- _ => apply (nonempty_linterp_linear_pure Pure) in H
  | H : _ |- _ => apply IHl1 in H
  | H : _ |- _ => apply IHl2 in H
  | H : _ |- _ => apply IHcont in H
  | H : context [Bits.single ?x] |- _ => destruct (Bits.single x)
  ...
  Lemma nonempty_interp_linear
  : ∀ E ty (l : linear _ E ty) (U : Update Σ) P
  ...
  repeat match goal with
  | H : ∃ _, _ |- _ => destruct H
  | H : _ |- _ => apply (nonempty_interp_atom_pure Pure) in H
  | H : _ |- _ => apply nonempty_interp_atom in H
  | H : _ |- _ => apply (nonempty_interp_linear_pure Pure) in H
  | H : _ |- _ => apply IHl1 in H
  | H : _ |- _ => apply IHl2 in H
  | H : _ |- _ => apply IHcont in H
  | H : context [Bits.single ?x] |- _ => destruct (Bits.single x)
