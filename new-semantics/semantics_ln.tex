...
\begin{align*}
	...
	\step(\ptree,p) \triangleq & [p_1\mapsto\{\ctx|\ctx\in \ptree(p)\}]                                                                                                                                                     & \text{when }\plabel{p}{p_1\:p_2}                                 \\
...
\[\Abs{\ptree}\triangleq\lfp(\lambda\Abs\ptree.\Abs\Step(\Abs\ptree)\sqcup\Abs{\ptree}_{\text{init}})\quad\text{where }\ctx_0\le(\Abs{\ptree}_{\text{init}}(p_0).1,\Abs{\ptree}_{\text{init}})\]
...
This is defined by
\[(\Abs{\ctx}_0,\Abs{\ptree}_0)\Abs\semlink\Abs\ptree\triangleq\lfp(\lambda\Abs{\ptree}_+.\Abs\Step(\Abs\Link(\Abs{\ctx}_0,\mathcal{\event}(\Abs\ptree),\Abs{\ptree}_+))\sqcup\Abs{\ptree}_0\sqcup\mathcal{V}(\Abs\ptree))\]
where
\[\mathcal{\event}(\Abs\ptree)\in\mathbb{P}\rightarrow\pset(\Abs\Event)^2\quad\mathcal{V}(\Abs\ptree)\in\Abs{\mathbb{T}}\]
are defined by
\[\mathcal{\event}(\Abs\ptree)\triangleq\lambda p.(\Abs\ptree(p).1.2,\Abs\ptree(p).2.1.2)\]
and
\[\mathcal{V}(\Abs\ptree)\triangleq\lambda p.((\Abs\ptree(p).1.1,\varnothing),((\Abs\ptree(p).2.1.1,\varnothing),\Abs\ptree(p).2.2))\]
